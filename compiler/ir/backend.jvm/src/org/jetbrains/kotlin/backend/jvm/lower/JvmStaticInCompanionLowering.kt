...
private class SingletonObjectJvmStaticLowering(val context: JvmBackendContext) : ClassLoweringPass {
    override fun lower(irClass: IrClass) {
        ...
        irClass.simpleFunctions().filter(::isJvmStaticDeclaration).forEach { jvmStaticFunction ->
...
internal fun IrDeclaration.isJvmStaticInSingleton(): Boolean {
    val parentClass = parent as? IrClass ?: return false
    return isJvmStaticDeclaration(this) && parentClass.isObject && !parentClass.isCompanion
}

private class MakeCallsStatic(val context: JvmBackendContext) : IrElementTransformerVoid() {
    override fun visitMemberAccess(expression: IrMemberAccessExpression<*>): IrExpression {
        val callee = expression.symbol.owner
        if (callee is IrDeclaration && callee.isJvmStaticInSingleton() && expression.dispatchReceiver != null) {
...
private fun isJvmStaticDeclaration(declaration: IrDeclaration): Boolean =
